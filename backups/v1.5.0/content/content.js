/**
 * BLANK FBA SCOUT - Content Script v1.5.0
...
(function () {
  ...
  /**
   * Extract ASIN with Multi-layer 2026 DOM Fallbacks
   */
  function extractAsin() {
    // 1. Direct URL regex match
    ...
    if (canonical && canonical.href) {
      const canonMatch = canonical.href.match(/(?:dp|gp\/product)\/([A-Z0-9]{10})/i);
      if (canonMatch && canonMatch[1]) return canonMatch[1].toUpperCase();
    }

    // 3. Form input or data attribute
    const asinInput = document.getElementById('ASIN') || 
                      document.querySelector('input[name="ASIN"]') || 
                      document.querySelector('#currentAsin') ||
                      document.querySelector('[data-asin]');
    if (asinInput) {
      const val = asinInput.value || asinInput.getAttribute('data-asin');
  ...
  /**
   * Extract Price with 2026 Layout Support
   */
  function extractPrice() {
    ...
    // Fallback: Check whole and fraction elements if offscreen span is hidden/absent
    ...
    const fracEl = document.querySelector('#corePriceDisplay_desktop_feature_div .a-price-fraction') || 
                   document.querySelector('.priceToPay .a-price-fraction') || 
                   document.querySelector('#corePrice_feature_div .a-price-fraction');
    if (wholeEl && wholeEl.innerText) {
      const w = wholeEl.innerText.replace(/[^0-9]/g, '');
      const f = fracEl && fracEl.innerText ? fracEl.innerText.replace(/[^0-9]/g, '') : '00';
  ...
  function extractSellerInfo() {
    ...
    // Method A: Direct link in the Buy Box (#sellerProfileTriggerId or a[href*="seller="] / /sp? / /shops/)
    const buyboxSellerLink = document.querySelector(`
      ...
      div[data-csa-c-slot-id*="buybox"] a[href*="seller="],
      #desktop_qualifiedBuyBox a[href*="seller="],
      #desktop_buybox a[href*="/sp?"],
